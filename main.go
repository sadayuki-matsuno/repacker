...
func run(argDir string) (err error) {
	...
	g.generateHead(dstPkg.name, srcType.dir, rootDir)
	if _, err = g.generate(srcType, dstType, rootDir); err != nil {
		return errors.Wrapf(err, "generate: %s", err)
	}

	var formatOnly bool
	if rootDir == srcType.dir {
		formatOnly = true
	}
	// Format the output.
...
func (g *Generator) generateHead(pkgName, importPath, rootDir string) {
	g.Printf("// Code generated by \"repacker %s\"; DO NOT EDIT\n", strings.Join(os.Args[1:], " "))
	g.Printf("\n")
	g.Printf("package %s", pkgName)
	g.Printf("\n")
	if rootDir != importPath {
...
func (g *Generator) generate(srcType, dstType Type, rootDir string) (funcName string, err error) {
	...
	if srcType.isSlice {
		return g.generateSliceCode(src, dst, rootDir), nil
	}

	return g.generateCode(src, dst, rootDir), nil
}
...
func (g *Generator) generateCode(src, dst Object, rootDir string) (funcName string) {
	...
	fmt.Fprintf(&code, "// %s creates %s from %s\n", funcName, dst.Name(), src.FullName(rootDir))
	fmt.Fprintf(&code, "func %s (s %s) %s {\n", funcName, src.FullName(rootDir), dst.Name())
		...
		for j := 0; j < dstInternal.NumFields(); j++ {
			...
			if srcField.Name() == dstField.Name() || (srcTagFound && dstTagFound && (srcTag == dstTag)) {
				...
				if !reflect.DeepEqual(srcField.Type(), dstField.Type()) {
					...
					default:
						nestedFuncName, err := g.generate(nestedSrcType, nestedDstType, rootDir)
						if err != nil {
...
func (g *Generator) generateSliceCode(src, dst Object, rootDir string) (funcName string) {
	nestedFunc := g.generateCode(src, dst, rootDir)
	...
	var code bytes.Buffer
	fmt.Fprintf(&code, "// %s creates []%s from []%s\n", funcName, dst.SliceName(), src.SliceFullName(rootDir))
	fmt.Fprintf(&code, "func %s (s []%s) (d []%s) {\n", funcName, src.SliceFullName(rootDir), dst.SliceName())
